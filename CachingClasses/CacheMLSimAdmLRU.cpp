#include "CacheMLSimAdmLRU.h"
#include <iostream>

using std::cerr;
using std::endl;

CacheMLSimulatorAdmLRU::CacheMLSimulatorAdmLRU(uint64_t _cache_size, uint64_t _wing) :
    CacheSim(_cache_size),
    wing_size(_wing),
    last_dim(2 * _wing + 1)
{}

double CacheMLSimulatorAdmLRU::predict_eviction(p::list& eviction_features) {
    prediction_updated_eviction = true;
    return vector<double>(to_std_vector<double>(eviction_features))[1];
}

bool CacheMLSimulatorAdmLRU::predict_admission(p::list& admission_features) {
...
void CacheMLSimulatorAdmLRU::produce_new_cache_state(p::dict &request, p::list& eviction_features, p::list& admission_features) {
