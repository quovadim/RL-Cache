...
def train_model(percentile, model, rewards, states, actions,
    ...
    while len(elite_states) == 0:
        elite_states, elite_actions, percentile_est, samples, mean_actions = select_elites(
            states, actions, rewards, percentile, max_samples)
                ...
                for name in keys_to_print:
                    if name == best_performance or name == worst_performance:
                        if name == best_performance:
                            print_list.append('\033[92m{:s}\033[0m : \033[1m{:5.2f}%\033[0m')
                        else:
                            print_list.append('\033[91m{:s}\033[0m : \033[1m{:5.2f}%\033[0m')
                    else:
                        print_list.append('\033[94m{:s}\033[0m : \033[1m{:5.2f}%\033[0m')
...
def generate_session_continious(
    ...
    for i in range(len(rows)):
        ...
        if decorrelated_metric and not exchanged:
            if hit:
                hit_metric_history_bhr[change_point - i - 1] = float(rows[i]['size']) * multiplier
                hit_metric_history_ohr[change_point - i - 1] = 1 * multiplier
            total_metric_history_bhr[change_point - i - 1] = float(rows[i]['size']) * multiplier
    ...
    if decorrelated_metric:
        ...
        total_metric_history_bhr = byte_reward_hits + byte_reward_miss + np.cumsum(total_metric_history_bhr)
        total_metric_history_ohr = reward_hits + reward_miss + np.cumsum(total_metric_history_ohr)
        ...
        for i in range(change_point):
            if i in lstates or i in lstates_adm:
                bhr = hit_metric_history_bhr[change_point - i - 1] / total_metric_history_bhr[change_point - i - 1]
                ohr = hit_metric_history_ohr[change_point - i - 1] / total_metric_history_ohr[change_point - i - 1]
                if (i in lstates_adm) and collect_admission:
                    admission_rating[lstates_adm.index(i)] = metric_funct(bhr, ohr, alpha)
