...
def metric(algorithm):
    return algorithm.byte_hit_rate() * alpha + algorithm.hit_rate() * (1 - alpha)


def eviction_rating_metric(algorithm):
    return algorithm.byte_eviction_rating() * alpha + algorithm.eviction_rating() * (1 - alpha)
...
def generate_session_continious(
    ...
    for i in range(len(rows)):
        algorithm.decide(rows[i], (predictions_evc[i]).tolist(),
                         (predictions_adm[i]).tolist())
...
class GameEnvironment:
    ...
    def run_and_train(self,
        ...
        q_percentile_admission = None
        q_percentile_eviction = 99
        ...
        for iteration in range(iterations):
            ...
            algorithms_ml = {'ML': algorithm,
                             'DET ML': generative_class(self.cache_size, self.wing_size)}
